...
def get_solar_azimuth(utc_time, lon, lat):
    """Return sun azimuth from *utc_time*, *lon*, and *lat*.
    lon,lat in degrees
    """
    lon = np.deg2rad(lon)
    lat = np.deg2rad(lat)
    ...
    ra_, dec = sun_ra_dec(utc_time)
    h__ = _local_hour_angle(utc_time, lon, ra_)
    sea, sa = get_alt_az(utc_time, lon, lat)

    soa = sa

    #soa = np.arccos(((np.sin(dec)*np.cos(lat))-(np.cos(h__)*np.cos(dec)*np.sin(lat)))/np.cos(sea));

    # soa = np.arcsin((-np.sin(h__)*np.cos(dec))/np.cos(sea))

    """
    soa = h__ * 180.0 / M_PI + 180.0;
    soa_new = soa
    i = np.where(soa > 360.0)
    soa_new[i] = soa[i] - 360.0
    j = np.where(soa < 0.0)
    soa_new[j] = soa[j] + 360.0;
    """
    return soa
