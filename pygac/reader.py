...
class Reader(object):
    ...
    def compute_lonlat(self, utcs=None, clock_drift_adjust=True):
        ...
        if clock_drift_adjust:
            ...
            try:
                offset_times, clock_error = get_offsets(
                ...
                offset_times = np.array(
                    offset_times, dtype='datetime64[ms]')
        ...
        if "constant_yaw_attitude_error" in self.head.dtype.fields:
            ...
            rpy = np.deg2rad([self.head["constant_roll_attitude_error"] / 1e3,
                              self.head[
                                  "constant_pitch_attitude_error"] / 1e3,
    ...
    def correct_times_median(self, year, jday, msec):
        ...
        msec = np.where(np.logical_and(np.logical_or(if_wrong_msec < -1000,
                                                     if_wrong_msec > 1000), if_wrong_jday != 1), msec[0] + msec_lineno, msec)
    ...
    def correct_scan_line_numbers(self, plot=False):
        ...
        diffs = np.abs(
            self.scans["scan_line_number"] - (ideal + med_offset))
    ...
    def correct_times_thresh(self, max_diff_from_t0_head=6 * 60 * 1000,
        ...
        if np.any(np.diff(nums) < 0):
            ...
            # We could already return here, but continue for plotting
            # purpose
        ...
        #
        # 1) Compute offsets between actual timestamps and idealized
        # timestamps
        offsets = t - tn
        ...
        if apply_corr:
            corrupt_lines = np.where(
                np.fabs(t - tn) > max_diff_from_ideal_t)
            self.utcs[corrupt_lines] = tn[corrupt_lines].astype(dt64_msec)
            LOG.debug("Corrected %s timestamp(s)",
                      str(len(corrupt_lines[0])))
        ...
        if plot:
            ...
            ax1.fill_between(
                along_track,
                t0_head - np.ones(along_track.size) *
                max_diff_from_t0_head,
                t0_head + np.ones(along_track.size) *
                max_diff_from_t0_head,
