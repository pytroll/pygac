...
import calibrate_pod as cal_pod
import h5py
import gac_io
...
def main(filename):
    ...
    #print head["number_of_scans"]

    packed_data = scans["sensor_data"]
    gac_counts = np.zeros((int(head["number_of_scans"]), 409 * 5))
    ...
    arrLat = np.zeros((float(head["number_of_scans"]), 51))
    arrLon = np.zeros((float(head["number_of_scans"]), 51))
    ...
    arrSZA = np.zeros((int(head["number_of_scans"]), 409))
    for i in range(head["number_of_scans"]):
    ...
    arrSAA = np.zeros((int(head["number_of_scans"]), 409))
    for i in range(head["number_of_scans"]):
            temp_utc = datetime.datetime(int(arrYear[i]), 1, 1) + datetime.timedelta(int(arrJDay[i]) - 1) + datetime.timedelta(milliseconds=int(arrUTC[i]));	
    ...
    arrSTA = np.zeros((int(head["number_of_scans"]), 409), dtype='float')
    for i in range(head["number_of_scans"]):
            temp_utc = datetime.datetime(int(arrYear[i]), 1, 1) + datetime.timedelta(int(arrJDay[i]) - 1) + datetime.timedelta(milliseconds=int(arrUTC[i]));	            
    ...
    arrRAA = np.zeros((int(head["number_of_scans"]), 409));
    ...
    channel3_switch=np.zeros(int(head["number_of_scans"]));
    channel3_switch=0;
    ref1,ref2,ref3=cal_pod.calibrate_solar_pod(gac_counts, int(arrYear[0]), int(arrJDay[0]), int(head["noaa_spacecraft_identification_code"]), channel3_switch, corr, int(head["number_of_scans"]));
    

    # Calibrating thermal channels

    decode_tele=np.zeros((int(head["number_of_scans"]),140));
    j=0;
    for i in range (35):
    ...
    ict_counts=np.zeros((int(head["number_of_scans"]),3));
    ...
    space_counts=np.zeros((int(head["number_of_scans"]),3));
    ...
    bt3=cal_pod.calibrate_thermal_pod(gac_counts[:,2::5], prt_counts, ict_counts[:,0], space_counts[:,0], int(head["number_of_scans"]), int(head["noaa_spacecraft_identification_code"]), channel=3);
    bt4=cal_pod.calibrate_thermal_pod(gac_counts[:,3::5], prt_counts, ict_counts[:,1], space_counts[:,1], int(head["number_of_scans"]), int(head["noaa_spacecraft_identification_code"]), channel=4);
    bt5=cal_pod.calibrate_thermal_pod(gac_counts[:,4::5], prt_counts, ict_counts[:,2], space_counts[:,2], int(head["number_of_scans"]), int(head["noaa_spacecraft_identification_code"]), channel=5);
    ...
    scanline_quality=np.zeros((int(head["number_of_scans"]),3));
