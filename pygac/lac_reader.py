...
"""Generic reader for LAC data. Can't be used as is, has to be subclassed to add
...
import datetime
from pygac.lac_calibration import calibrate_solar, calibrate_thermal
...
class LACReader(object):
    ...
    def get_counts(self):
        ...
        gac_counts = np.zeros((len(self.scans), 2048 * 5))
        gac_counts[:, 0::3] = (packed_data >> 20) & 1023
        gac_counts[:, 1::3] = ((packed_data >> 10) & 1023)[:, :-1]
        gac_counts[:, 2::3] = (packed_data & 1023)[:, :-1]	
        gac_counts = gac_counts.reshape((-1, 2048, 5))
        ...
        try:
            ...
            channels = np.zeros((len(self.scans), 2048, 6),
    ...
    def compute_lonlat(self, utcs=None, clock_drift_adjust=True):
        ...
        rows = -(len(self.scans["scan_line_number"]))

        return lons.reshape(rows, 51), lats.reshape(rows, 51)
